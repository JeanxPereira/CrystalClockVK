...
void main()
{
    ...
    if (renderPass == 0) {
        // PASS 1: Base transparent geometry
        vec3 result = CalcDirLight(dirLight, fragNormal, viewDir) * 0.6;
        vec3 rim = CalcRimLight(viewDir, fragNormal);
        gl_FragColor = vec4(mix(rim, result, 0.8), rodAlpha);
    }
    else if (renderPass <= 2) {
        // PASS 2-3: Colored/lit rods + Offset rotation
        ...
        vec3 rim = CalcRimLight(viewDir, fragNormal);
        gl_FragColor = vec4(mix(rim, result, 0.7) * rodAlpha, rodAlpha);
    }
    else {
        // PASS 4-5: Selected rod highlight / fill
        vec3 result = material.ambient * 1.5 + CalcDirLight(dirLight, fragNormal, viewDir);
        for (int i = 0; i < NR_POINT_LIGHTS; i++) {
            result += CalcPointLight(pointLights[i], norm, tanFragPosition, tanViewDir) * 0.5;
        }
        vec3 rim = CalcRimLight(viewDir, fragNormal);
        gl_FragColor = vec4(mix(rim, result, 0.6) * rodAlpha, rodAlpha);
    }
}
