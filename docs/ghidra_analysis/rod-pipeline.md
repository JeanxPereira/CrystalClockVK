...
| `FUN_00232e38` | `0x00232e38` | large | **draw_crystal_rod** — confirmed anchor. `swc1 f0,0x4(s2)` at `0x232e48` writes `clockState[0x1b]` (global intensity float, loaded in caller at `0x002240ac`) to `rod+0x04` each frame — NOT the angle (W0-1 finding, see `w0-projection-constants.md §4`). Also zeroes `rod+0x08`. Calls `rotation_build @ 0x002732d8`, `projection_build @ 0x002730a8`, `matrix_multiply @ 0x002738a0` (proj × rot). |
