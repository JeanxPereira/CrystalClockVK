...
base 0x00200000, **currently active**). [DECOMP-SOURCED]
- **Clock addresses (0x0022xxxx) resolve in `OSDSYS.elf`.** [DECOMP-SOURCED] Decompiled the rod render loop
  (function `FUN_00225be8`, contains the 0x00225E80 ~~the docs call "master render"~~
  [FALSIFIED → item 1: 0x00225E80/FUN_00225be8 is NOT the per-frame master render; real
  per-frame rod render is 0x00232618, live BP 2026-06-12 — see "Live render-chain correction"
  section below]). The GS style is literally in the pseudocode — no interpretation needed:
  ...
  - This **confirms `GS_FIXED_POINT_SCALE = 16`** (cut plan Task 1), not the old 65536. [DECOMP-SOURCED]
- **OPERATIONAL NOTE:** active program is `hddosd.elf`. Every clock decompile MUST pass
  `program="OSDSYS.elf"` explicitly or it silently targets the wrong binary. [LIVE-VERIFIED]
- Ghidra `executable_path` shows macOS paths (`/Users/jeanxpereira/...`) — project was authored
  on macOS, now bridged on this Windows box. Decompilation works regardless. [LIVE-VERIFIED]
...
- `asm/clock/` — render funcs incl. `clock_orb_rendering_func.s`. [DECOMP-SOURCED]
- `asm/graph/` — the style spec, already isolated as named functions:
  ...
  `vif1SetAlphaBlend.s`, `vif1SetTEST_1.s`, `vif1SetZTest.s`, `gsAllocBuffer.s`. [DECOMP-SOURCED]
  `pktSetAlphaBlend` builds GS A+D packets for registers 0x42 (ALPHA) — readable, currently
  `nonmatching` (disassembled, not yet C-matched). [DECOMP-SOURCED]
- `src/` — partial C ports exist (cdvd full, clock partial). Most render funcs still asm-only;
  Ghidra pseudocode is the practical read path until matched. [DECOMP-SOURCED]
- Build needs `ee-gcc 2.9-991111` (`$MATCH_CC`) for byte-match; modern ps2dev won't match.
  Source binary: `OSDSYS_A_XLF_decrypted_unpacked.elf`. [DECOMP-SOURCED]
...
**`C:\CodingProjects\Personal\PCSX2-MCP\pcsx2-mcp-server\dist\index.js`** (the user's bundle). [LIVE-VERIFIED]
After a Claude restart the tools loaded and connected live: DebugServer (127.0.0.1:21512) +
Pine (28011). [LIVE-VERIFIED] Verified end-to-end: pause, read_registers (128-bit), read_memory, disassemble,
execution breakpoints (idle BP fired; clock-render BP fired). [LIVE-VERIFIED] Game ID `20080220` confirms the
v2.30 USA BIOS match. [LIVE-VERIFIED]
- **MUST run the bundled patched `C:\CodingProjects\Personal\PCSX2-MCP\pcsx2-qt.exe`** — it
  carries the DebugServer patch. A stock PCSX2 has no DebugServer and will never connect. [LIVE-VERIFIED]
- **GOTCHA:** do NOT reset the emulator with OSDSYS execution BPs armed — during early boot
  ...
  the screen-build code. The crash is transient; reset again and it boots clean. [LIVE-VERIFIED]
...
from-source RenderDoc build against modern Python. [LIVE-VERIFIED] Not worth it for a debug-convenience tool
...
reference screenshot — the ground-truth visual + primitive pair): [DUMP-MEASURED]
- **`clock_viewer.gs`** (6.17 MB, from `..._203208.gs.zst`) — **the crystal clock (Visor mode):
  radial ring of prism rods around the central glow. THIS IS THE PHASE-1 TARGET.** [DUMP-MEASURED]
- `config_menu.gs` (6.33 MB, from `..._203126.gs.zst`) — System Config screen (floating cubes,
  selected cube glows). Deferred / future scope. [DUMP-MEASURED]

Both are valid PCSX2 GSDump: serial `20080220-175343` (matches our BIOS), screenshot 640×480
(PS2 NTSC), ~1.2 MB GS state freeze + GIF transfer stream. [DUMP-MEASURED] **Ground-truth pipeline
(capture → decompress → parse) proven viable.** No zstd CLI / python-zstandard on the box —
decompress via Node 26 `zlib.zstdDecompressSync` (`/tmp/unzst.mjs`). [LIVE-VERIFIED]
...
[FALSIFIED → item 1: confirmed correction — see live BP 2026-06-12].
That address (and 0x00225be8) NEVER execute per-frame — they are builder/init or stale. [LIVE-VERIFIED]
The **real per-frame rod render is `0x00232618`** (BP hit at +0x28 = 0x00232640), reached via
this backtrace: [LIVE-VERIFIED]
```
0x00221408 → 0x00221060 → 0x00221558 → 0x0022e738 → 0x0022c8d0 → 0x00233928 → 0x00232618
```
<!-- [LIVE-VERIFIED] backtrace captured via live BP hit, 2026-06-12 -->
Live registers at the hit (use to re-seed Phase-1 RE; heap addresses are per-boot): [LIVE-VERIFIED]
- `s3 = 0x00375250` = ROD_GROUP_A_ADDR ✅ (MEMORY §7 correct here, confirmed live) [LIVE-VERIFIED]
- `s0 = 0x20297220` = GS packet build buffer (uncached; matches the `DAT_20297xxx` writes in the decompile) [LIVE-VERIFIED]
- `v1 = 0x1000A000` = VIF1 FIFO → render submits via VIF1/GIF DMA [LIVE-VERIFIED]
- `gp = 0x002CFEF0` ⚠️ ≠ MEMORY §7's ~~`0x002AF070`~~ [FALSIFIED → item 9: gp=0x002AF070 is stale;
  live/correct gp is 0x002CFEF0] — every GP-relative ABSOLUTE address in §7
  (the angle-step / refraction-displacement float table) must be RE-RESOLVED; only the offsets are stable. [LIVE-VERIFIED]
- Rod group @0x375250 holds float positions (~-11.4, 47.6) + 12.4 fixed-point screen coords
  (`0x784d`/16 = 1924.8px, matching float `0x44f09a18`). [LIVE-VERIFIED]
...
Vulkan SDK paths (macOS), pcsx2-mcp `D:\` path, decomp `D:\` references. [LIVE-VERIFIED] PCSX2 likely needs
Windows setup. [HYPOTHESIS]
