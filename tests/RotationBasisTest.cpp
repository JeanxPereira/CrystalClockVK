...
void assertOrthonormal(const ps2clock::Mat4& m, const std::string& tag) {
    ps2clock::Vec3 c0 = ps2clock::Vec3(glm::column(m, 0));
    ps2clock::Vec3 c1 = ps2clock::Vec3(glm::column(m, 1));
    ps2clock::Vec3 c2 = ps2clock::Vec3(glm::column(m, 2));
    ...
    check(near(glm::determinant(ps2clock::Mat4(ps2clock::Vec4(c0, 0), ps2clock::Vec4(c1, 0),
                                            ps2clock::Vec4(c2, 0), ps2clock::Vec4(0, 0, 0, 1))),
               1.0f),
          tag + " right-handed det=+1");

    // W column is (0,0,0,1) — no translation in the rotation matrix.
    ps2clock::Vec4 c3 = glm::column(m, 3);
...
int main() {
    ...
    assertOrthonormal(ps2clock::BuildRotationMatrix(ps2clock::Vec3(0, 0, -1), ps2clock::Vec3(0, 1, 0)),
                      "forward=-Z up=+Y");

    // Skewed inputs (up not perpendicular to forward) must still orthonormalize.
    assertOrthonormal(ps2clock::BuildRotationMatrix(ps2clock::Vec3(1, 0.2f, -0.3f), ps2clock::Vec3(0.1f, 1, 0)),
                      "skewed");

    // Arbitrary rod-ish direction.
    assertOrthonormal(ps2clock::BuildRotationMatrix(ps2clock::Vec3(-13.0f, 14.0f, 50.0f), ps2clock::Vec3(0, 1, 0)),
                      "rod0 direction");

    // NaN guard (bc1t -> memclr): degenerate input (forward parallel to up) zeroes the matrix.
    ps2clock::Mat4 z = ps2clock::BuildRotationMatrix(ps2clock::Vec3(0, 1, 0), ps2clock::Vec3(0, 1, 0));
